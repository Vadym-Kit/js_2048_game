...
function render(board) {
    ...
    if (game.getScore() >= 0) score.textContent = game.getScore();
 ...
 */ class Game {
   ...
   */ constructor(initialState){
        ...
        this.size = 4;
        this.tokens = [];
        for(let i = 0; i < 4; i++)this.tokens[i] = Array(this.size).fill(0);
    ...
    moveLeft() {
        ...
        for (const row of notEmpty)while(row.length < this.size)row.push(0);
    ...
    moveRight() {
        ...
        notEmpty = notEmpty.map((row)=>row.filter((n)=>n > 0));
        for (const row of notEmpty)while(row.length < this.size)row.unshift(0);
        const untill = JSON.stringify(this.board);
        this.board = notEmpty;
        const afterr = JSON.stringify(this.board);
        this.lose(untill, afterr);
    ...
    moveUp() {
        ...
        for(let col = 0; col < this.size; col++){
            const colArray = [];
            for(let row = 0; row < this.size; row++)colArray.push(this.board[row][col]);
            notEmpty.push(colArray);
        }
        notEmpty = notEmpty.map((col)=>col.filter((n)=>n > 0));
        notEmpty.forEach((col)=>{
            for(let i = 0; i < col.length - 1; i++)if (col[i] === col[i + 1]) {
                col[i] *= 2;
                this.score += col[i];
                col[i + 1] = 0;
            }
        ...
        notEmpty = notEmpty.map((col)=>col.filter((n)=>n > 0));
        for (const col of notEmpty)while(col.length < this.size)col.push(0);
    ...
    moveDown() {
        ...
        notEmpty = notEmpty.map((col)=>col.filter((n)=>n > 0));
        notEmpty.forEach((col)=>{
            for(let i = col.length - 1; i > 0; i--)if (col[i] === col[i - 1]) {
                col[i] *= 2;
                this.score += col[i];
                col[i - 1] = 0;
            }
        ...
        notEmpty = notEmpty.map((col)=>col.filter((n)=>n > 0));
        for (const col of notEmpty)while(col.length < this.size)col.unshift(0);
        notEmpty = this.columnsToRows(notEmpty);
        const untill = JSON.stringify(this.board);
        this.board = notEmpty;
        const afterr = JSON.stringify(this.board);
    ...
    lose(beforeMove, afterMove) {
        if (beforeMove !== afterMove) this.randomAdd();
        for (const row of this.board){
            if (row.includes(0)) return;
        }
        for(let row = 0; row < this.size; row++)for(let col = 0; col < this.size; col++){
            const value = this.board[row][col];
            if (col + 1 < this.size && this.board[row][col + 1] === value) return;
            if (row + 1 < this.size && this.board[row + 1][col] === value) return;
        }
        this.status = "lose";
    }
    columnsToRows(columns) {
        ...
        for(let col = 0; col < size; col++)for(let row = 0; row < size; row++)rows[row][col] = columns[col][row];
        return rows;
    }
    // moveX() {
   ...
   */ start() {
        this.board = [];
        for(let i = 0; i < 4; i++)this.board[i] = Array(this.size).fill(0);
        this.score = 0;
        this.status = "playing";
        this.randomAdd();
        this.randomAdd();
